...
class User(pydantic.BaseModel):
    ...
    def password_valid(cls, value):
        ...
        if any(d in value for d in string.digits) and any(l in value for l in string.ascii_lowercase) \
        and any(u in value for u in string.ascii_uppercase) and any(p in value for p in string.punctuation):
            s = "[@_!#$%^&*()<>?}{~:]"
            for x in range(len(value)):
                if value[x] in s:
                    return value
            ...
            raise ValueError(
                "Password must contain at least one punctuation symbol, digit, uppercase letter, lowercase letter, special character")
